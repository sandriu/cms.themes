<?php
    $expanded = FALSE;
    if (!empty($data['regions']) || !empty($data['countries']) || !empty($data['instrument']) || !empty($data['mailing_list']) ||
        !empty($data['availability']) || !empty($data['person_status']) || !empty($data['organization_status'])) {
        $expanded = TRUE;
    }
